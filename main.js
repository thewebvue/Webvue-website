...
/* ---------- contact form + WhatsApp/Call handlers ---------- */
...
/* ---------- dynamic gallery routing & theme switching ---------- */
function switchGalleryView(targetViewId, themeName) {
  // 1. Hide all views
  const allViews = document.querySelectorAll('.gallery-view');
  allViews.forEach(view => {
    view.classList.add('hidden');
    view.classList.remove('block', 'animate-fade-in');
  });
  ...
  if (targetView) {
    ...
    targetView.classList.add('block', 'animate-fade-in');
  }

  // 3. Apply the colorful background theme to the section
  const categorySection = document.getElementById('categories');
  if (categorySection && themeName) {
    categorySection.setAttribute('data-theme', themeName);
  }

  // 4. Scroll smoothly to the top of the gallery
